-- Solution 1: Drop existing tables and recreate (CAUTION: This deletes all data!)

-- Drop tables in reverse order (to handle foreign key constraints)
DROP TABLE IF EXISTS password_changes CASCADE;
DROP TABLE IF EXISTS debt_documents CASCADE;
...
-- Now run the complete schema from the previous message
-- (Copy the entire schema from the previous response and paste it here)
