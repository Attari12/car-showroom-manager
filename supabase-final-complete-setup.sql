...
-- WITH INVESTORS & SELLERS TRACKING
...
-- Enhanced cars table with investment tracking
CREATE TABLE cars (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    client_id UUID NOT NULL REFERENCES clients(id) ON DELETE CASCADE,
...
CREATE INDEX idx_cars_buyer_id ON cars(buyer_id);
CREATE INDEX idx_cars_seller_id ON cars(seller_id);
CREATE INDEX idx_cars_ownership_type ON cars(ownership_type);
CREATE INDEX idx_dealers_client_id ON dealers(client_id);
CREATE INDEX idx_buyers_client_id ON buyers(client_id);
CREATE INDEX idx_investors_client_id ON investors(client_id);
CREATE INDEX idx_sellers_client_id ON sellers(client_id);
CREATE INDEX idx_car_investments_car_id ON car_investments(car_id);
CREATE INDEX idx_car_investments_investor_id ON car_investments(investor_id);
CREATE INDEX idx_car_investments_is_active ON car_investments(is_active);
CREATE INDEX idx_profit_distributions_car_id ON profit_distributions(car_id);
CREATE INDEX idx_buyer_debts_client_id ON buyer_debts(client_id);
CREATE INDEX idx_buyer_debts_buyer_id ON buyer_debts(buyer_id);
CREATE INDEX idx_buyer_debts_is_settled ON buyer_debts(is_settled);
CREATE INDEX idx_dealer_debts_client_id ON dealer_debts(client_id);
CREATE INDEX idx_dealer_debts_dealer_id ON dealer_debts(dealer_id);
CREATE INDEX idx_dealer_debts_is_settled ON dealer_debts(is_settled);
...
BEGIN
    ...
    (test_client_id, 'Ahmed Ali', 'ahmed.investor@email.com', '+92-300-1234567', 'Lahore, Pakistan', '42101-1234567-1', 500000, 75000, 2),
    ...
    -- Insert sample cars with comprehensive data including investment tracking
    INSERT INTO cars (id, client_id, make, model, year, owner_name, registration_number, color, condition, mileage, purchase_price, asking_price, purchase_date, status, description, dealer_id, buyer_id, dealer_commission, seller_id, showroom_investment, ownership_type, commission_type, commission_amount, commission_percentage) VALUES
    (uuid_generate_v4(), test_client_id, 'Toyota', 'Corolla GLi', 2020, 'Original Owner', 'LEA-1234', 'Pearl White', 'Excellent', 45000, 3200000, 3500000, '2023-01-15', 'available', 'Well maintained family car with complete service history. Non-accidental, first owner, all documents clear.', dealer1_id, NULL, 100000, seller1_id, 2000000, 'partially_owned', 'flat', 0, 0),
    (uuid_generate_v4(), test_client_id, 'Honda', 'Civic Oriel', 2019, 'Company Vehicle', 'LEB-5678', 'Silver Metallic', 'Good', 52000, 2800000, 3100000, '2023-02-20', 'sold', 'Clean corporate car with minor wear and tear. Regular maintenance done.', dealer2_id, buyer1_id, 150000, seller2_id, 0, 'fully_investor_owned', 'percentage', 0, 15),
    (uuid_generate_v4(), test_client_id, 'Suzuki', 'Alto VXR', 2021, 'Lady Driven', 'LEC-9012', 'Candy Red', 'Excellent', 28000, 1800000, 2000000, '2023-03-10', 'available', 'Brand new condition with remaining warranty. Perfect for city driving.', NULL, NULL, NULL, NULL, 1800000, 'partially_owned', 'flat', 0, 0),
    (uuid_generate_v4(), sigma_client_id, 'BMW', 'X3 xDrive30i', 2018, 'Import Direct', 'LED-3456', 'Jet Black', 'Good', 68000, 7500000, 8200000, '2023-04-05', 'reserved', 'Luxury SUV with premium features. Imported from Japan with complete auction sheet.', dealer3_id, NULL, 300000, NULL, 4000000, 'partially_owned', 'flat', 0, 0),
    (uuid_generate_v4(), sigma_client_id, 'Mercedes-Benz', 'C200 AMG Line', 2019, 'Lease Return', 'LEE-7890', 'Brilliant Blue', 'Excellent', 42000, 6200000, 6800000, '2023-04-15', 'sold', 'Executive sedan in pristine condition. Full service history available.', dealer3_id, buyer4_id, 250000, NULL, 3000000, 'partially_owned', 'flat', 0, 0)
    RETURNING id INTO car1_id;
    
    -- Get car IDs for investment tracking
    SELECT id INTO car1_id FROM cars WHERE registration_number = 'LEA-1234' AND client_id = test_client_id;
    SELECT id INTO car2_id FROM cars WHERE registration_number = 'LEB-5678' AND client_id = test_client_id;
    SELECT id INTO car3_id FROM cars WHERE registration_number = 'LEC-9012' AND client_id = test_client_id;
    SELECT id INTO car4_id FROM cars WHERE registration_number = 'LED-3456' AND client_id = sigma_client_id;
    SELECT id INTO car5_id FROM cars WHERE registration_number = 'LEE-7890' AND client_id = sigma_client_id;
    ...
    (car1_id, investor1_id, 1200000, 37.5, true), -- 1.2M out of 3.2M = 37.5%
    (car2_id, investor2_id, 2800000, 100, false), -- Fully investor owned (sold)
    (car1_id, investor2_id, 800000, 25, true), -- Second investor in first car
    (car4_id, investor3_id, 3500000, 46.7, true), -- BMW investor
    (car5_id, investor3_id, 3200000, 51.6, false) -- Mercedes investor (sold)
    ON CONFLICT (car_id, investor_id) DO NOTHING;
    
    -- Insert profit distributions for sold cars
    ...
    (car2_id, 3100000, 300000, 45000, 'commission', 255000, '{"commission_rate": 15, "investor_share": 255000, "showroom_commission": 45000}'),
    (car5_id, 6800000, 600000, 294000, 'ownership', 306000, '{"showroom_ownership": 48.4, "investor_ownership": 51.6, "showroom_profit": 294000, "investor_profit": 306000}')
    ON CONFLICT DO NOTHING;
    
    -- Add more cars for better inventory display
    INSERT INTO cars (client_id, make, model, year, owner_name, registration_number, color, condition, mileage, purchase_price, asking_price, purchase_date, status, description, dealer_id, buyer_id, dealer_commission, seller_id, showroom_investment, ownership_type) VALUES
    (test_client_id, 'Toyota', 'Camry Grande', 2017, 'Bank Auction', 'LEB-2468', 'Champagne Gold', 'Good', 89000, 4200000, 4600000, '2023-05-01', 'available', 'Mid-size luxury sedan with leather interior. Needs minor touch-up.', dealer1_id, NULL, 120000, seller1_id, 4200000, 'partially_owned'),
    (test_client_id, 'Honda', 'City Aspire', 2018, 'Doctor Owner', 'LEC-1357', 'Taffeta White', 'Excellent', 71000, 2400000, 2750000, '2023-05-10', 'available', 'Well-maintained sedan with genuine parts only. Complete record available.', NULL, NULL, NULL, seller2_id, 2400000, 'partially_owned'),
    (test_client_id, 'Suzuki', 'Cultus VXL', 2020, 'Expat Owned', 'LEA-9753', 'Graphite Grey', 'Excellent', 35000, 1650000, 1850000, '2023-05-15', 'pending', 'Low mileage hatchback. Expat leaving country sale.', dealer2_id, NULL, 80000, seller1_id, 1000000, 'partially_owned'),
    (sigma_client_id, 'Audi', 'A4 TFSI', 2017, 'Businessman', 'LED-8642', 'Glacier White', 'Good', 95000, 5800000, 6300000, '2023-05-20', 'available', 'German engineering with quattro AWD. Premium interior with sunroof.', dealer3_id, NULL, 200000, seller3_id, 3500000, 'partially_owned'),
    (sigma_client_id, 'Toyota', 'Land Cruiser VX', 2016, 'Embassy Vehicle', 'LEE-1975', 'Desert Beige', 'Good', 125000, 12500000, 13800000, '2023-05-25', 'available', 'Bulletproof SUV with premium features. Diplomatic use with maintenance records.', NULL, NULL, NULL, NULL, 12500000, 'partially_owned')
    ON CONFLICT DO NOTHING;
    ...
    RAISE NOTICE 'Enhanced sample data inserted successfully with investor and seller tracking';
END $$;
...
-- Create new policies (allow all operations for simplicity)
...
-- View for car summary with investment details
...
SELECT 
    ...
    COALESCE(ci.total_investor_investment, 0) as total_investor_investment,
    COALESCE(ci.investor_count, 0) as investor_count,
    (c.showroom_investment + COALESCE(ci.total_investor_investment, 0)) as total_investment
FROM cars c
...
LEFT JOIN (
    SELECT 
        car_id,
        SUM(investment_amount) as total_investor_investment,
...
-- View for investor performance
CREATE OR REPLACE VIEW investor_performance AS
SELECT 
    i.*,
    COALESCE(active.active_amount, 0) as current_active_investment,
    COALESCE(completed.total_profit_earned, 0) as lifetime_profit_earned,
    COALESCE(completed.completed_investments, 0) as completed_investment_count,
    CASE 
        WHEN i.total_investment > 0 THEN (i.total_profit / i.total_investment * 100)
        ELSE 0 
...
LEFT JOIN (
    SELECT 
        investor_id,
        SUM(investment_amount) as active_amount
    FROM car_investments 
    WHERE is_active = true
    GROUP BY investor_id
) active ON i.id = active.investor_id
LEFT JOIN (
    SELECT 
        investor_id,
        SUM(profit_earned) as total_profit_earned,
        COUNT(*) as completed_investments
    FROM car_investments 
    WHERE is_active = false
    GROUP BY investor_id
) completed ON i.id = completed.investor_id;

-- View for monthly profit summary
...
SELECT 
    ...
    SUM(showroom_profit) as total_showroom_profit,
    SUM(investor_profit) as total_investor_profit,
    AVG(total_profit) as avg_profit_per_car
...
SELECT 'Cars by Ownership Type' as overview;
SELECT ownership_type, COUNT(*) as count FROM cars GROUP BY ownership_type ORDER BY ownership_type;

SELECT 'Investment Summary' as overview;
SELECT 
    c.username,
    COUNT(cars.*) as total_cars,
    SUM(cars.showroom_investment) as showroom_investment,
    COALESCE(SUM(ci.total_investor_investment), 0) as investor_investment,
    (SUM(cars.showroom_investment) + COALESCE(SUM(ci.total_investor_investment), 0)) as total_investment
FROM clients c 
LEFT JOIN cars ON c.id = cars.client_id 
LEFT JOIN (
    SELECT car_id, SUM(investment_amount) as total_investor_investment
    FROM car_investments 
    GROUP BY car_id
) ci ON cars.id = ci.car_id
GROUP BY c.username 
...
BEGIN
    RAISE NOTICE '============================================';
    RAISE NOTICE 'COMPLETE CAR MANAGEMENT SYSTEM SETUP FINISHED!';
    RAISE NOTICE '============================================';
    RAISE NOTICE 'Tables Created:';
    RAISE NOTICE '  - Core: clients, cars, dealers, buyers';
    RAISE NOTICE '  - New: investors, sellers, car_investments, profit_distributions';
    RAISE NOTICE '  - Debts: buyer_debts, dealer_debts';
    RAISE NOTICE 'Features:';
    RAISE NOTICE '  - Investment tracking with multiple investors per car';
    RAISE NOTICE '  - Profit distribution calculations';
    RAISE NOTICE '  - Seller relationship management';
    RAISE NOTICE '  - Commission tracking (flat/percentage)';
    RAISE NOTICE '  - Comprehensive analytics views';
    RAISE NOTICE 'Sample Data: 3 clients, 10 cars, 3 dealers, 4 buyers, 3 investors, 3 sellers';
    RAISE NOTICE 'Storage: car-images, car-documents, debt-documents buckets';
    RAISE NOTICE 'Views: car_summary, investor_performance, monthly_profit_summary';
